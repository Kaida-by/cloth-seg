...
class AlignedDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        feet_channel = np.zeros((self.width, self.height), dtype=np.uint8)
        bag_channel = np.zeros((self.width, self.height), dtype=np.uint8)
        head_channel = np.zeros((self.width, self.height), dtype=np.uint8)
        ...
        for i in range(len(labels)):
            ...
            elif labels[i] in feet:
                feet_channel += new_masks[i]
            elif labels[i] in bag:
                bag_channel += new_masks[i]
            elif labels[i] in head:
                head_channel += new_masks[i]
        ...
        feet_channel = (feet_channel > 0).astype("uint8")
        bag_channel = (bag_channel > 0).astype("uint8")
        head_channel = (head_channel > 0).astype("uint8")

        final_label = first_channel + second_channel * 2 + third_channel * 3 + feet_channel * 4 + bag_channel * 5 + head_channel * 6
